...
def _mock_loader(mocker, calval_range, science_range):
    properties = {
        Mission.SWOT_SWATH_CALVAL: SimpleNamespace(
            date_start=calval_range[0], date_end=calval_range[1]
        ),
        Mission.SWOT_SWATH_SCIENCE: SimpleNamespace(
            date_start=science_range[0], date_end=science_range[1]
        ),
    }
    loader = mocker.Mock()
